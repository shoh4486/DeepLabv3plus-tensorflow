...
def lr_polynomial_decay(init_lr, global_step, decay_steps, end_lr=0.0, power=1.0, 
                        name='lr_polynomial_decay'):
    """
...
def miou_tf(pred, gt, keep_batch_dim=False):
    ...
    """
    intersection = tf.reduce_sum(pred*gt, axis=[1, 2]) 
    ...
    if not keep_batch_dim:
        ...
        miou = tf.reshape(tf.reduce_mean(iou, axis=-1), [-1, 1]) 
        # not averaged across the batch, shape: [N, 1]
        return miou
...
def pixel_acc_4D_tf(pred, gt, return_axis='ALL'):
    ...
    - pixel values must be either 0 or 1 (label).
    - return_axis: dimension to return ('BC', 'B', 'C', or 'ALL'; 
                   B and C denote batch and channel dimension, respectively.)
    """
    true_positive = tf.reduce_sum(pred*gt, axis=[1, 2])
    true_negative = tf.reduce_sum((1-pred)*(1-gt), axis=[1, 2])
    
    pixel_acc_batch_channel = tf.cast(true_positive + true_negative, \
              tf.float32)/tf.cast(tf.shape(pred)[1]*tf.shape(pred)[2], tf.float32)
    # calculated over spatial dimension, shape: [N, C]
    
    if return_axis == 'BC':
        return pixel_acc_batch_channel # shape: [N, C]
    elif return_axis == 'B':
        pixel_acc_batch = tf.reshape(tf.reduce_mean(pixel_acc_batch_channel, 
                                                    axis=-1), [-1, 1])
        return pixel_acc_batch # shape: [N, 1]
    elif return_axis == 'C':
        pixel_acc_channel = tf.reshape(tf.reduce_mean(pixel_acc_batch_channel, 
                                                      axis=0), [-1, 1])
        ...
        raise NotImplementedError('Parameter \'return_axis\' should be \
                                  one of \'BC\', \'B\', \'C\' and \'ALL\'.')
        
def l_relu(inputs, alpha=0.2, name='leaky_relu'):
    """
    Leaky ReLU
    (Maas, A. L. et al., Rectifier nonlinearities imporve neural network 
    acoustic models, Proc. icml. Vol.30. No.1. 2013)
    """
...
class Conv2D:
    ...
    """
    def __init__(self, FH=3, FW=3, weight_decay_lambda=None, 
...
class SepConv2D:
    ...
    """
    def __init__(self, FH=3, FW=3, CM=1, weight_decay_lambda=None, 
                 truncated=False, stddev=0.02):
        """
        Parameters
    ...
    def __call__(self, inputs, FN, s=1, rate=None, is_BN=True, is_training=True, 
                 name='sep_conv2d', padding='SAME', bias=False):
        """
        Parameters
        inputs: [N, H, W, C]
        ...
        with tf.variable_scope(name):
            ...
            if not self.weight_decay_lambda:
                w1 = tf.get_variable(name='depthwise_weight', shape=[self.FH, self.FW, C, self.CM], 
                                     dtype=tf.float32, initializer=initializer)
                w2 = tf.get_variable(name='pointwise_weight', shape=[1, 1, int(C*self.CM), FN], 
                                     dtype=tf.float32, initializer=initializer)
            else:
                w1 = tf.get_variable(name='depthwise_weight', shape=[self.FH, self.FW, C, self.CM], 
                                     dtype=tf.float32, initializer=initializer,
            ...
            h = []
            if rate is None:
                h.append(tf.nn.depthwise_conv2d(inputs, w1, strides=[1, sdy, sdx, 1], 
                                                padding=padding, rate=rate))
                # depthwise convolution (no atrous)
            elif rate == 1:
                h.append(tf.nn.depthwise_conv2d(inputs, w1, strides=[1, sdy, sdx, 1], 
                                                padding=padding, rate=[rate, rate]))
                # depthwise convolution (no atrous)
            else: # rate > 1
                h.append(tf.nn.depthwise_conv2d(inputs, w1, strides=[1, 1, 1, 1], 
                                                padding=padding, rate=[rate, rate]))
            ...
            if bias:
                b1 = tf.get_variable(name='depthwise_bias', shape=[C], dtype=tf.float32, 
                                     initializer=tf.constant_initializer(0.0))
                h.append(tf.reshape(tf.nn.bias_add(h[-1], b1), [-1, h[-1].get_shape()[1],
                                    h[-1].get_shape()[2], h[-1].get_shape()[3]]))
            ...
            if not bias:
                ...
                b2 = tf.get_variable(name='pointwise_bias', shape=[FN], dtype=tf.float32, 
                                     initializer=tf.constant_initializer(0.0))
                h.append(tf.reshape(tf.nn.bias_add(h[-1], b2), [-1, h[-1].get_shape()[1], 
                                    h[-1].get_shape()[2], h[-1].get_shape()[3]]))
                return h[-1]  
...
class Block:
    ...
    def __call__(self, inputs, FN1, FN2, FN3, first_relu=True, downsampling=False, rates=None, 
        ...
        rates: (int) dilation rates in three different seperable convs., 
               shape: [r1, r2, r3] (assume r1=rH1=rW1, r2=rH2=rW2, r3=rH3=rW3)
