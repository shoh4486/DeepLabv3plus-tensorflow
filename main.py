...
flags.DEFINE_bool('separable', True, 'applying separable convolution')
            ...
            for i in range(len(seg_test)):
                for c in range(FLAGS.num_class):
                    np.savetxt(os.path.join(FLAGS.save_dir, "test", "test_result%d_class%d.txt" % (i, c)), 
                               seg_test[i, :, :, c])
                    imsave(os.path.join(FLAGS.save_dir, "test", "test_result%d_class%d.png" % (i, c)),
                           seg_test[i, :, :, c], cmap=plt.cm.gray)
                #imsave(os.path.join(FLAGS.save_dir, "test", "test_result%d.png" % i), seg_test[i, :, :, :-1])
        else:
