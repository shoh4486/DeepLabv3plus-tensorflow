...
class Xception:
    ...
    """
    def __init__(self, is_training, weight_decay_lambda=None, truncated=False):
...
class ASPP:
    ...
    """
    def __init__(self, H, W, is_training, separable=True, drop_rate=0.5, weight_decay_lambda=None, truncated=False):
        """
        Parameters
...
class Decoder:
    ...
    def __init__(self, H, W, num_class, is_training, separable=True, 
                 drop_rate1=0.5, drop_rate2=0.1, weight_decay_lambda=None, truncated=False):
        """
        Parameters
        H, W: training image size
...
class DeepLabv3plus:
    """
    DeepLabv3+ by Chen, L.-C. et al., Encoder-Decoder with atrous separable convolution for semantic image segmentation,
    ...
    def build_model(self):
        ...
        with tf.name_scope('instantiations'):
            xception = Xception(self.is_training, self.weight_decay_lambda, self.truncated) 
            aspp = ASPP(self.H, self.W, self.is_training, self.separable_aspp_decoder, self.drop_rate0, 
                        self.weight_decay_lambda, self.truncated)
            decoder = Decoder(self.H, self.W, self.num_class, self.is_training, self.separable_aspp_decoder, 
                              self.drop_rate1, self.drop_rate2, self.weight_decay_lambda, self.truncated)
        ...
        with tf.name_scope('loss'):
            softmax_cee0 = tf.losses.softmax_cross_entropy(onehot_labels=self.gts, logits=de_aspp_x0) # output_stride=16
            softmax_cee1 = tf.losses.softmax_cross_entropy(onehot_labels=self.gts, logits=de_aspp_x1) # output_stride=8
        ...
        with tf.name_scope('performance_measures'):
            with tf.name_scope('error'): # as a loss checker / not used in the training phase
                self.softmax_cee0_ = tf.losses.softmax_cross_entropy(onehot_labels=self.gts, logits=de_aspp_x0) # output_stride=16
                self.softmax_cee1_ = tf.losses.softmax_cross_entropy(onehot_labels=self.gts, logits=de_aspp_x1) # output_stride=8
        ...
        tf.summary.image('Xception_result', tf.slice(x0, begin=[0, 0, 0, 0], 
                                                     size=[4, tf.cast(tf.ceil(self.H/16), tf.int32), tf.cast(tf.ceil(self.W/16), tf.int32), 1]), 
                         max_outputs=4)
    ...
    def train(self, inputs, gts, config): 
        ...
        for epoch in range(config.start_epoch, config.end_epoch):
            t1 = time.time()
            k = np.random.RandomState(seed=epoch).choice(len(H_list), total_train_num, replace=True)
            ...
            for kk in range(len(HW_list_)):
                hw_list.append(self.grid(H_orig, W_orig, HW_list_[kk][0], HW_list_[kk][1], random_pick=True, seed=epoch))
            ########################################
            
            ######################################## list4 for random flipping, len: total_train_num
            random_flip_list = np.random.RandomState(seed=epoch).choice([True, False], total_train_num, replace=True).tolist()
            ########################################
            
            batch_number = np.random.RandomState(seed=epoch).choice(total_train_num, (iters_per_epoch, config.batch_size_training), 
            ...
            for i, batch in enumerate(batch_number):
                ...
                for j in range(config.batch_size_training):
                    ...
                    tmp_inputs = inputs_train[train_index_list[batch[j]], rh:rh+rH, rw:rw+rW, :].reshape(1, rH, rW, -1)
                    tmp_gts = gts_train[train_index_list[batch[j]], rh:rh+rH, rw:rw+rW, :].reshape(1, rH, rW, -1)
                    
                    if random_flip_list[batch[j]]: # random flip
                        tmp_inputs = np.flip(tmp_inputs, axis=2)
                        tmp_gts = np.flip(tmp_gts, axis=2)
                    
                    tmp0.append(self.sess.run(tmp_nn, feed_dict={tmp_data: tmp_inputs})) # resizing to [H_train, W_train]
                    tmp1.append(self.sess.run(tmp_nn, feed_dict={tmp_data: tmp_gts}))

                inputs_batch = np.concatenate(tuple(tmp0), axis=0).reshape(config.batch_size_training, H_train, W_train, -1)
                gts_batch = np.concatenate(tuple(tmp1), axis=0).reshape(config.batch_size_training, H_train, W_train, -1)
                
                if config.random_brightness_contrast:
                    tmp2 = []
                    for image in inputs_batch:
                        tmp2.append(self.sess.run(tmp_random, feed_dict={tmp_data2: image})) # random brightness and random contrast   
                    inputs_batch = np.concatenate(tuple(tmp2), axis=0).reshape(config.batch_size_training, H_train, W_train, -1)
                t4 = time.time() # t4-t3 << 1 s
                
                if config.lr_decay: # dependent on end_epoch
                    lr_tmp = lr_polynomial_decay(config.lr_init, global_step=(epoch - config.start_epoch)*iters_per_epoch + i, # real-time iteration number
                                                 decay_steps=(config.end_epoch - config.start_epoch)*iters_per_epoch, # the number of entire iterations
                                                 end_lr=0.0, power=0.9)
                
                self.sess.run(train_step, feed_dict={self.inputs: inputs_batch, self.gts: gts_batch, 
                                                     self.H: H_train, self.W: W_train,
                                                     self.is_training: config.bn_training,
                                                     self.drop_rate0: 0.5, self.drop_rate1: 0.5, self.drop_rate2: 0.1,
                                                     self.lr: lr_tmp})
            #######################################
            
            if epoch % config.check_epoch == 0:
                self.seg_train, CEE_train_val, miou_train_val, PA_ALL_train_val, summary_train = self.sess.run([hardmax_de_aspp_x, softmax_cee, miou, PA_ALL, merge],
                                                                                                               feed_dict={self.inputs: inputs_train_, self.gts: gts_train_, 
                                                                                                                          self.H: H_train, self.W: W_train,
                ...
                self.seg_valid, CEE_valid_val, miou_valid_val, PA_ALL_valid_val = self.sess.run([hardmax_de_aspp_x, softmax_cee, miou, PA_ALL], 
                                                                                                feed_dict={self.inputs: inputs_valid, self.gts: gts_valid, 
                                                                                                           self.H: H_train, self.W: W_train,
                                                                                                           self.is_training: False,
                                                                                                           self.drop_rate0: 0.0, self.drop_rate1: 0.0, self.drop_rate2: 0.0})
    ...
    def evaluation(self, inputs, output_stride, gts=None): # at the validated epoch
        ...
        if output_stride == 16:
            if np.sum(gts == None):
                ...
                return self.sess.run([self.hardmax_de_aspp_x0, self.softmax_cee0_, self.miou0, self.PA_ALL0], 
                                     feed_dict={self.inputs: inputs, self.gts: gts,
                                                self.H: inputs.shape[1], self.W: inputs.shape[2],
                                                self.is_training: False,
                                                self.drop_rate0: 0.0, self.drop_rate1: 0.0, self.drop_rate2: 0.0})
            ...
            if np.sum(gts == None):
                ...
                return self.sess.run([self.hardmax_de_aspp_x1, self.softmax_cee1_, self.miou1, self.PA_ALL1], 
                                     feed_dict={self.inputs: inputs, self.gts: gts,
                                                self.H: inputs.shape[1], self.W: inputs.shape[2],
                                                self.is_training: False,
                                                self.drop_rate0: 0.0, self.drop_rate1: 0.0, self.drop_rate2: 0.0})